...
| `protectionMap` | `boolean` | on when esbuild's `sourcemap` is set |
| `sourceMap` | `boolean` | on in development when esbuild emits a map, off in production |
...
stronger protection. Keep the key out of your build script: the plugin rejects it there. See
