/**
 * Executes the OBFUSCATED main and preload bundles under plain node, against the
 * fixture's stub `electron` module, and asserts the app still behaves. The stub
 * routes `ipcRenderer.invoke` to whatever `ipcMain.handle` registered, so the
 * preload bundle's calls really reach the main bundle's handlers -- the same
 * boundary the real runtime crosses, minus the display server CI does not have.
 *
 * Standalone by design (spawned by electron-app.spec.ts): a fresh process per run means
 * no ESM module cache carrying a previous build's bundles.
 */
...
// `app.whenReady().then(...)` is a microtask; let it settle before asserting.
...
// The `activate` handler must re-create a window only when none are open --
// obfuscated control flow over `BrowserWindow.getAllWindows()`.
...
// The fail path must survive obfuscation too: a non-finite amount throws out of
// the main handler and rejects the invoke.
let threw = "";
try {
